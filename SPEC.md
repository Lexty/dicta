...
| a watcher goes away mid-stream | — | the daemon drops it and keeps serving; **no dictation outcome changes**. A UI is an observer, and an observer that vanishes is not an event the thing it was observing has to notice (D27). The daemon notices the close, or a byte arriving after the handshake was read, **without writing anything**, idle included, and frees the watcher's slot at once. A write that fails is not how a dead watcher is found: an idle daemon writes nothing, so watchers that died while it was idle would hold their slots until every new one was refused |
| the watcher cap is reached | — | the new watcher is refused by an ordinary short response, **never by a dropped connection** — a client reads a close as a dead daemon, which would make a healthy refusal read as a crash. The watchers already attached are untouched and **no dictation outcome changes**. The menu draws the refusal **as a refusal, never as "not answering"**: amber, with the daemon's reason and "Restart dicta", because the daemon answered; and it retries on the same bounded backoff, so it attaches once a slot frees |
...
| the menu cannot read the record | — | **no dictation outcome changes**: the record is written by the daemon, and a reader failing is not an event the writer has to notice. The panel keeps the last good rows on screen under an amber label, "The record could not be read: <reason>", and **never says "Nothing yet."**, which is a claim about what the record holds and not about a read that failed. The reason is the reader's own, without the path, which is always the same known file. A later successful read clears the label, and a read started earlier that lands later never overrides a newer one, whichever of the two failed (D27) |
