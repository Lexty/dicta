...
| second daemon instance attempted | `test: a second daemon on a live socket refuses to start`, `test: a second daemon is refused while the first one's socket is live`, `test: a socket left by a crashed daemon is replaced rather than refused`. Before `setup.json` is touched, by the lock: `test: a second contender is refused as already running while the first holds the lock`, `test: contenders starting together: exactly one takes the lock, every other is refused`, taken again once the owner lets go or drops it, `test: once the owner lets go the next daemon takes it, and the file is never removed` and `test: a lock dropped without release, as a daemon that exits drops it, is taken again`, never inherited by a child, `test: the lock's descriptor is closed on exec, so no child can keep it held`, and a lock that cannot be asked for told apart from a running daemon, `test: a lock that cannot be created is a failure of its own, never already running`, `test: a lock file nobody may open, in a writable directory, is cannot-open, not a pass` and `test: a flock that fails for another reason is cannot-lock, and releases its descriptor`, while `test: an existing lock file in a read-only directory is still taken`; that start-up refuses on every one of those failures before the store, the switch and the frontmost source, `test: start-up refuses on any lock failure before it builds, reads or writes anything`, with `SetupStore.bootstrap` taking the lock as an argument so that nothing reaches it without one, and still probes the socket before the bootstrap, `test: a refused second daemon is refused before it bootstraps setup.json` |
...
| hold key pressed while agterm is not frontmost, with the scope not `other-apps` | `test: with the gate closed, the hold key does nothing at all in another application`. The assertion is **silence**, not merely "no dictation": the trigger sends nothing and the notifier logs nothing, because a notification here would fire on every right-Control combination typed in a browser. Its opposite number, so that the two are not confused, is `test: a session that cannot be resolved starts nothing and is said out loud` |
...
| a hold on the focused-field path outlasts the floor without the Accessibility grant | `test: with the grant missing, a threshold is silent, sends nothing, and reports the grant`; and the daemon's own check behind it, which refuses out loud before capture if the grant went between the threshold and the start: the `noGrant` case of `test: a field start is refused before capture opens, for every reason it can be refused`. **H28** (a) for the hold being silent with the microphone never lit and the banner amber, (b) for the grant picked up without a restart, and (e) for silence under a Focus mode too; **H38** (b) for a revoked grant |
...
| a chord, or the hold key in front of agterm, while `agtermctl` is absent and the scope is `other-apps` | `test: with no agterm, a chord and a focus start are refused with a reason naming agtermctl` for the refusal, opening nothing, and `test: with no agterm, untargeted refusals notify through system feedback` for where it is said when agterm cannot say it. That the daemon starts at all is `test: a missing agtermctl is survived under every scope, and fatal only under --no-hold`, and that `--no-hold` is the one exit, `test: a missing agtermctl is fatal under --no-hold, whatever the flag`; that readiness does not block dictation over it, `test: a missing agterm is a fault only under agterm-only` and, through the daemon, `test: with no agterm, readiness blocks dictation only for a daemon without focused fields`; and that the menu shows a notice rather than a fault, `test: a missing agterm with focused fields on is drawn as a working daemon, not a fault` and `test: a missing agterm with focused fields on is an amber notice, and without them a fault`. The agent that runs it is `test: the agent rendered for both focused-field settings lints, with no empty argument`. **H33** (a) and (b) score start-up both ways and (c) both refusals on a real machine, while a hold in VS Code still dictates |
| `configure` cannot write `setup.json`, including a failed replacement of an unreadable one | `test: a configure whose write fails changes nothing, is refused, and publishes the reason`, with the scope and the gate unchanged and the save error cleared by the next write that succeeds; over an unreadable file and one written by a newer build, `test: a configure over an unusable setup.json replaces it, and a failed one keeps the problem`, where the failed replacement leaves `setup.json` byte-identical and publishes the load problem beside the save error, and the retry replaces again and clears both; the same test carries both snapshots to the setup window, the problem screen with the error beside it and then the checklist with none. That a save error is shown inline and changes neither the screen nor its controls, `test: a save error is shown inline on the screen the table picks, with that screen's controls`. **H42** (b): with the support directory read-only, choosing again leaves the window on the problem with the write error beside it and the scope unchanged, and (c) the retry that succeeds keeps the original as `setup.json.unreadable` |
| `setup.json` unreadable, or written by a newer build | Read and kept: `test: each unreadable kind loads as its problem, and the file is byte-identical afterwards`, `test: a setup.json that cannot be opened is unreadable, never absent` and `test: an unreadable setup.json at start-up writes nothing and reports the problem`, which also holds that it behaves as `agterm-only`; choosing again keeps the original, `test: a replacement keeps the unreadable original as setup.json.unreadable, then writes` and `test: a replacement never has a moment without setup.json`; a symbolic link to nothing kept as itself, `test: a setup.json that is a symbolic link to nothing is unreadable, never migrated over`, and left as it was when the rename fails, `test: a failed rename leaves a dangling symlink as it was, and still unreadable`; a directory named as one and never touched, `test: a directory at setup.json is named, and a choice over it fails without touching it`; and a replacement that fails never becomes a migration, even with the flag still given, `test: a failed replacement step leaves setup.json byte-identical, and never migrates`. What the log says about it, `test: the start-up lines name a setup problem: an unusable file, or a first write that failed`. That the window opens by itself on it and draws the problem, `test: an idle first snapshot opens the window only for a choice, the offer or a problem` and `test: a load problem is the problem screen, whatever the save error says`. End to end, **H42** (a): agterm dictation still works, a hold in VS Code does nothing, no accessibility dialog appears, the log names the problem, and the window opens on it; (c) choosing again keeps the original. The one case in which a config file blocks a dictation is scored by the same item on a machine without agterm, (d) |
| `accessibility` asked for while the scope is not `other-apps` | `test: accessibility is refused with zero calls unless the scope is other-apps`, with and without `prompt`, including a gate a choice has closed again; that nothing else in the daemon asks, start-up included, `test: start-up never asks for the grant: only the accessibility verb with prompt does`. **H36** (d): `dictactl accessibility --prompt` under `agterm-only` is refused, and no dialog appears |
| the Accessibility grant is revoked while the scope is `other-apps` | **H38**: (a) noticed when the person comes back to the window, (b) a silent hold in VS Code with an amber banner and no window opening by itself, and no timer anywhere between. That coming back to the window checks the grant and never prompts, `test: becoming key checks the grant only under other apps, and never prompts` and `test: becoming key right after Allow Access was clicked still never prompts`; that no window opens by itself after the first snapshot of a launch, `test: a snapshot that is not the launch's first never opens the window` and `test: the latch never fires again, across a reconnect and a window close`. That a missing grant is a pending step and never a fault, `test: a missing grant blocks only without agterm, and is never a fault`, and that its banner is amber and opens the setup window, `test: a banner is red only for a fault, and a setup step is amber with Set Up`. That the hold goes silent rather than saying so, `test: with the grant missing, a threshold is silent, sends nothing, and reports the grant`; that nothing but an asked-for check looks at the grant, `test: no timer checks the grant: the log holds exactly the checks that were asked for`; and that a check admitted before a close or a reopening cannot overwrite what a later one found, `test: a grant report from a closed gate or an earlier generation never reaches readiness` |
| the menu-bar UI is not running while a choice is pending | **H36** (e): with the menu booted out and the offer unanswered, agterm dictation works and nothing asks for Accessibility; the offer appears when the menu is started again. The daemon's half needs no menu: an unanswered offer is `agterm-only`, under which `accessibility` makes no call, `test: accessibility is refused with zero calls unless the scope is other-apps`, and start-up asks for nothing, `test: start-up never asks for the grant: only the accessibility verb with prompt does`; with no choice and no agterm the socket is served and waits, `test: with no choice and no agterm, the socket is served, setup waits, and nothing asks`. That the offer still opens by itself at the first snapshot of the menu started later, `test: an idle first snapshot opens the window only for a choice, the offer or a problem` |
| `configure` during a dictation | `test: configure agterm-only during a field dictation affects only the next start`: the live attempt delivers in full through the handle and the injector it was accepted with, its final validation still checks the grant, and the next field start is refused with no accessibility call. That a choice applies without a restart, `test: configure other-apps persists, then opens the gate, then publishes, with no restart`. **H39** (b): a `configure` landing while a hold in VS Code is still recording leaves that dictation delivered in full, and the next hold follows the new scope |
| two setup choices made before the first reaches the daemon | `test: conflicting choices, the first held in transport, arrive in click order; the last wins`, which holds the first request inside its transport, and asserts the daemon's arrival order, its writes and the scope and gate in force; a close behind a pending click, `test: a close enqueued behind a pending click is sent after it, never before`; and a new worker, still in order, once the sender has drained, `test: a sender that has drained starts a new worker for what is enqueued later, in order`. Not reachable by hand on purpose: the window is the time a thread takes to connect |
| the gate closes between the threshold's gate read and its grant check | `test: a gate closing during the threshold's grant check discards the report; nothing follows`, closed and closed-then-reopened; a start already on its way refused by the daemon's own read of the gate with no further call, `test: a start already sent when the gate closes is refused by the daemon, with no more calls`; a gate closed before the threshold, `test: a gate closed before the threshold abandons the hold silently, with no AX call`; and a started hold still stopped through what it captured, `test: a started field hold is stopped after the gate closes, through what it captured`. Not reachable by hand: the window between the two reads is microseconds wide, so **H39** (b) scores the neighbouring case a hand can reach, a `configure` landing around a live hold, where the next hold follows the new scope and nothing is typed twice |
...
| a watcher goes away mid-stream | `test: a watcher that goes away is dropped, and the daemon keeps serving` is the daemon surviving it; `test: a watcher on the far end does not change what an attempt does` is the half the row actually claims — that no OUTCOME moves. The two are separate on purpose: a daemon that kept serving while quietly changing what a dictation did would pass the first and fail the invariant the row exists for. How the daemon notices, with nothing written: the first test publishes nothing, so a slot it frees was freed by the read side; `test: watchers that die while the daemon is idle free their slots for the next one` is the cap of dead watchers giving way to a live fifth; `test: a watcher that talks after the handshake has its watch ended and its slot freed` is the unsolicited byte; and `test: a live watcher that says nothing is never taken for a dead one` is the guard, which a read side that took silence for EOF would fail. On the installed pair, with a menu killed while the daemon is idle, **H49** |
| the watcher cap is reached | `test: past the cap a watcher is REFUSED with a response, never by a dropped connection`, and `test: the watcher cap exists and is small`. The wording of the first is the finding: `ControlClient` reads a closed connection as `daemonCrashed`, so refusing by hanging up would make a healthy daemon that is answering correctly report itself as dead in the one window the user would look at. The other half is the menu drawing that refusal as one: `test: a refused watch is amber, keeps the daemon's reason, and never says it is not answering`, `test: a refused watch is drawn as a refusal, and every other thrown error still as a failure`, and `test: a refused watch retries through after, and a later accepted watch connects` |
...
so its row names the nearest item a hand can score beside the tests that drive it.
...
- **H35 — a fresh install chooses from a window, and dictates without a restart.** On a clean macOS
  ...
  an Accessibility dialog before (b). Scored together with **H48**: (a) is the window that crashed
  the menu before this item could be scored.
...
- **H41 — the window never takes focus in the middle of a session.** The rule being scored: the
  ...
  agterm for a few minutes. Pass: it never reopens by itself. Scored together with **H48**, whose (a)
  is the same launch.
...
- **H47 — the setup window opens through its presenter at exactly the doors it had.** With a
  ...
  and "Set Up…" still brings it forward. Scored together with **H48**: every door here opens the
  window H48 keeps alive.
