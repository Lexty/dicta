...
struct RecentRow: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 6) {
            // The dot is the outcome, and it is the same vocabulary the glyph uses: green landed,
            // amber landed with something worth knowing, red did not arrive, faint had nothing in
            // it. One meaning per colour across the whole family (`docs/ui-vocabulary.md`).
            Circle()
                .fill(row.tint.color)
            ...
            VStack(alignment: .leading, spacing: 1) {
                primary
                Text(row.secondary(at: now))
                    .font(.caption2)
                    .foregroundStyle(.secondary)
                // The sentence the user was already shown, from §9's own `error` field — so this
                // and the notification cannot word one event two ways. Tinted like the dot, because
                // an amber reason ("typed, filter skipped") and a red one ("the target is gone")
                // are different news and the row should not make them look alike.
